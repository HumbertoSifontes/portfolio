...
export const navData = [
  ...
  {name: "contactame", path: "/contact", icon: <HiEnvelope /> },
];
...
const Nav = () => {
  ...
  return (
    <nav className='flex flex-col items-center xl:justify-center gap-y-4 fixed h-max 
                    bottom-0 mt-auto xl:right-[2%] z-40 top-0 w-full xl:w-16 xl:max-w-md xl:h-screen'>
      <div className="flex w-full xl:flex-col items-center justify-between xl:justify-center gap-y-10 
                      px-4 md:px-40 xl:px-0 h-[80px] xl:h-max py-8 
                      bg-white/10 backdrop-blur-sm text-3xl xl:text-xl xl:rounded-full">
        {navData.map((link, index) => {
          return (
            <Link 
              className={`${link.path === pathname && 'text-accent animate-pulse'} relative flex items-center group icon hover:scale-125 transition-all duration-300`} 
              href={link.path} 
              key={index}
              >
